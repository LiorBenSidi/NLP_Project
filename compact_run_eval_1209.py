...
import time
from compact_evaluation_1209 import evaluate_reports
    ...
    #         all_true_reports_data = json.load(f)
        ...
        for game_key, game_narrative_data in all_examples_data.items():
            ...
            if llm_report:
                ...
                # discrepancies — פעם אחת תחת per_game
                results_by_difficulty[difficulty]["per_game"][game_key]["discrepancies"] = disc

                # details — קובץ אחד לכל משחק (רק אם det קיים)
                if det is not None:
                ...
                # מילוי התוצאות לשתי השיטות (לשמירת תאימות לסיכומים)
                for et in EVAL_TYPES:  # ["field", "fractional_per_block"]
                    acc_pct = totals_by_type[et]["accuracy_pct"]
                    formula = totals_by_type[et]["formula"]

                    # סטטיסטיקה מצטברת ל-summary
