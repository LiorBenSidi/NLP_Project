...
import time
from evaluation_1209 import evaluate_reports
    ...
    #         all_true_reports_data = json.load(f)
        ...
        for lvl in ["basic", "medium", "hard"]:
            results_by_difficulty[lvl] = {
                ...
                "details": {et: {} for et in EVAL_TYPES},
                "per_game": {}
            }
        ...
        for game_key, game_narrative_data in all_examples_data.items():
            ...
            for attempt in range(max_retries):
                ...
                time_after_response = 60 * minutes # 60 seconds
            ...
            # שמור נתיב לקובץ הגולמי בפר-משחק
            ...
            if llm_report:
                ...
                # הוסף גם את נתיב ה-JSON המתוקן לפר-משחק
                ...
                per_type_results = {}
                # ודא שיש סל ל-per_game עבור המשחק הזה
                results_by_difficulty[difficulty]["per_game"].setdefault(game_key, {})

                for et in EVAL_TYPES:
                    res = evaluate_reports(llm_report, ground_truth_data, eval_type=et, return_details=RETURN_DETAILS)
                    ...
                    per_type_results[et] = (acc, disc, det)
                    ...
                    if det:
                        results_by_difficulty[difficulty]["details"][et][game_key] = det

                        # שמירת קובץ פירוט מלא למשחק+שיטה
                        details_path = os.path.join(
                            results_base_dir, difficulty, "json", f"{game_key}__{et}_details.json"
                        )
                        with open(details_path, "w", encoding="utf-8") as fdet:
                            json.dump(det, fdet, indent=4, ensure_ascii=False)
                        print(f"[{et}] Saved detailed evaluation to: {details_path}")

                        # סיכום קליל לפר-משחק (דיוק + נוסחה)
                        totals = det.get("totals", {})
                        results_by_difficulty[difficulty]["per_game"][game_key][et] = {
                            "accuracy_pct": acc,
                            "correct_fields": totals.get("correct_fields"),
                            "total_fields": totals.get("total_fields"),
                            "formula": totals.get("formula")
                        }
                    else:
                        # אם אין details, עדיין נשמור את הדיוק
                        results_by_difficulty[difficulty]["per_game"][game_key][et] = {
                            "accuracy_pct": acc
                        }

                # הדפסה נקייה למסך כולל נוסחה + שם השיטה + sanity למצב fractional
                for et, (acc, disc, det) in per_type_results.items():
                    et_label = et
                    if isinstance(det, dict):
                        et_label = det.get("eval_type", et)

                    if isinstance(det, dict) and det.get("totals"):
                        totals = det["totals"]
                        cf = totals.get("correct_fields")
                        tf = totals.get("total_fields")
                        formula = totals.get("formula")

                        extra = ""
                        if et_label == "fractional_per_block":
                        ...
                        print(f"--- RESULT for {game_key} ({et_label}): Accuracy = {acc:.2f}% | calc: {formula} (= {cf}/{tf}){extra} ---")
                    else:
                        print(f"--- RESULT for {game_key} ({et_label}): Accuracy = {acc:.2f}% ---")
            ...
            for difficulty, results in results_by_difficulty.items():
                ...
                for et in EVAL_TYPES:
                    ...
                    per_type_summary[et] = {
                        ...
                        "details": results["details"].get(et, {})
                    }
